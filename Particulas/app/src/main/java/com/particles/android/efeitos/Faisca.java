...
public class Faisca {
    ...
    // -- [ MÉTODO CONSTRUTOR ] --
    public Faisca(Geometry.Point posicao, Geometry.Vector direcao, int cor, float angulo, float velocidade)
    {
    ...
    public void addParticulas(SistemaParticulas sistemaParticulas, float tempoAtual, int quantidade) {
        for (int i = 0; i < quantidade; i++) {
            ...
            setRotateEulerM(matrizDeRotacao, 0, (random.nextFloat() - 0.5f) * angulo, (random.nextFloat() - 0.5f) * angulo, (random.nextFloat() - 0.5f) * angulo);
            ...
            sistemaParticulas.addParticula(new Geometry.Point(posicao.x,posicao.y,posicao.z), cor, direcaoParticula, tempoAtual);
        }
